...
ncfile = Dataset("/mnt/depo2/WRFOUT/hava12/wrfout_d01_2021-01-11_18.nc")
...
for i, time in zip(range(len(times)), times):
    ...
    levels = [0, .1, 0.2, .4, .6, .8, 1., 1.5, 2., 3., 5., 8., 12., 16., 20., 25., 30., 40.]
    norm, cmap = colortables.get_with_boundaries('precipitation', levels)
    CF = bm.contourf(x, y, rain, levels,  norm=norm, cmap=cmap, extend='max')
    ...
    # plt.savefig('testrain_'+str(stime)+'.png')
    plt.show()
    plt.close(fig)
    #exit()
